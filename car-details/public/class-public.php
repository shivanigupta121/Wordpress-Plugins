...
add_filter('the_content', 'display_car_listing_shortcode_on_selected_page');
add_filter('the_content', 'display_car_shortcode_on_selected_page');
add_action('wp_enqueue_scripts', 'enqueue_ajax_script');
add_shortcode('car_listing', 'car_list_table');
add_shortcode('car_add', 'car_add_form');
add_action('wp_ajax_save_car_data', 'save_car_data');
add_action('wp_ajax_nopriv_save_car_data', 'save_car_data');

// Function to enable or disable the shortcode based on settings
function display_car_shortcode_on_selected_page($content) {
...
// Function to enable or disable the shortcode based on settings
function display_car_listing_shortcode_on_selected_page($content) {
    ...
    <table class="custom-car-list-table">
        ...
        <tbody>
            ...
            while ($car_posts->have_posts()) : $car_posts->the_post();
                ...
                echo '<a class="custom-view-button" href="' . $view_link . '">View</a>';
                echo '<a class="custom-edit-button" href="#">Edit</a>';
...
function car_add_form() {
    ...
    <form id="car-add-form" method="post" enctype="multipart/form-data">
        ...
        <div class="custom-form-group">
            ...
            <?php
                $car_description = ''; // Initialize an empty description
                wp_editor($car_description, 'car_description', array(
                    'textarea_name' => 'car_description', // Specify the name
                    'textarea_rows' => 4,
                ));
            ?>
        </div>
...
function save_car_data() {
    ...
    $car_image = upload_car_image(); // You should have your upload_car_image() function defined.
